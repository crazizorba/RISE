...
class FSDPActor(FSDPModelManager, Worker):
    ...
    def run_inference(
        ...
        while recv_batch_size < self.total_batch_size_per_dp:
            ...
            # ! Important ref_policy_state_dict
...
class EmbodiedFSDPActor(FSDPModelManager, Worker):
    def __init__(self, cfg: DictConfig):
        ...
        self.use_ema_quantile = cfg.actor.model.get("use_ema_quantile", False)
        if self.use_ema_quantile:
            self.ema_quantile = EMAQuantile(beta=0.95, quantile=0.8)
    ...
    def _process_received_rollout_batch(
        ...
        if (
            ...
            wm_dones = rollout_batch[
                "wm_steps"
            ]  # [n_chunk_step, rollout_epoch x bsz, num_action_chunks]
            loss_mask, loss_mask_sum = compute_loss_mask(torch.logical_or(dones,wm_dones))
            # ?? Bug here: only compute the first dimension of action chunks
            # * Not bug since we do not use this for IL.
    ...
    def run_training(self):
        ...
        self.model.train()
        rollout_size = (
            self.rollout_batch["prev_logprobs"].shape[0]     # * 4
            * self.rollout_batch["prev_logprobs"].shape[1]   # * 32
        )
        g = torch.Generator()
        ...
        for _ in range(update_epoch):
            ...
            for train_global_batch in rollout_dataloader_iter:
                ...
                for idx, data in enumerate(train_micro_batch):
                    ...
                    with self.amp_context:
                        output_dict = self.model(  # * go to OpenPiForRLActionPrediction, forward func
                            data=data,
                    ...
                    if self.cfg.actor.model.openpi.train_mode == 'IL':
                        ...
                        loss = il_loss
                        metrics_data = {}
                        metrics_data["il_loss"] = il_loss.detach().item()
                    else:
...
class EMAQuantile:
    def __init__(self, beta=0.95, quantile=0.8):
        """
        EMA Quantile Updater
        args:
            beta: Smoothing coefficient (the closer to 1, the more dependent on historical values, and the smaller the fluctuation)
            quantile: Target quantile
        """
        self.beta = beta
        self.quantile = quantile
        self.ema_q = 0.0  # Initialize EMA quantiles
    
    def update(self, batch_advantages, is_expert):
        """Update EMA quantiles"""

        # Extract the positive dominance value of the current batch
        batch_positive = batch_advantages[(~is_expert) & (batch_advantages > 0)]
        if len(batch_positive) == 0:
            return  # No positive advantage, no updates
        
        # Calculate the quantiles of the current batch
        batch_q = torch.quantile(batch_positive, self.quantile).item()
        # EMA update
        self.ema_q = self.beta * self.ema_q + (1 - self.beta) * batch_q
    
    def get_ema_threshold(self):
        return self.ema_q

def calibrate_advantage_ema(ema_quantile, advantages, is_expert, alpha=0.8):
    # Update EMA quantiles
    ema_quantile.update(advantages, is_expert)
    ema_threshold = ema_quantile.get_ema_threshold()
    
    # Adaptive calibration
    positive_mask = (~is_expert) & (advantages > 0)
    high_uncertainty_mask = positive_mask & (advantages >= ema_threshold)
    low_uncertainty_mask = positive_mask & (advantages < ema_threshold)
    
    calibrated = advantages.clone()
    calibrated[high_uncertainty_mask] *= alpha
    calibrated[low_uncertainty_mask] *= (alpha + 0.2)
