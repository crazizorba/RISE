...
class PI0Pytorch(nn.Module):
    ...
    def gradient_checkpointing_enable(self):
        ...
        # Gọi hàm kích hoạt chuẩn của Hugging Face cho từng thành phần mô hình
        # Gọi hàm tắt gradient checkpointing chuẩn của Hugging Face
        if hasattr(self.paligemma_with_expert.paligemma, "gradient_checkpointing_disable"):
            ...
            self.paligemma_with_expert.paligemma.model.gradient_checkpointing = False

        self.paligemma_with_expert.paligemma.model.config._attn_implementation = "eager"
        
        # Đường dẫn chuẩn hóa nấc trung gian .language_model để tìm thấy mảng .layers
        self.paligemma_with_expert.paligemma.model.language_model.layers[0].self_attn.q_proj.weight.dtype == torch.bfloat16
    ...
    def sample_actions(self, device, observation, noise=None, num_steps=10, 
        ...
        self.paligemma_with_expert.paligemma.model.config._attn_implementation = "eager"  # noqa: SLF001
    ...
    def denoise_step(
        ...
        forced_mask = torch.zeros((full_att_2d_masks_4d.shape[0], 1, full_att_2d_masks_4d.shape[2], 807), 
                                  dtype=full_att_2d_masks_4d.dtype, 
                                  device=full_att_2d_masks_4d.device)
        outputs_embeds, _ = self.paligemma_with_expert.forward(
            attention_mask=None,#full_att_2d_masks_4d,
            position_ids=position_ids,
    ...
    def sample_values(self, device, observation) -> Tensor:
        ...
        try:
            if hasattr(self.paligemma_with_expert.paligemma, "language_model"):
                is_bfloat16 = (self.paligemma_with_expert.paligemma.language_model.model.layers[0].self_attn.q_proj.weight.dtype == torch.bfloat16)
            elif hasattr(self.paligemma_with_expert.paligemma.model, "layers"):
                is_bfloat16 = (self.paligemma_with_expert.paligemma.model.layers[0].self_attn.q_proj.weight.dtype == torch.bfloat16)
            else:
                is_bfloat16 = (next(self.paligemma_with_expert.paligemma.parameters()).dtype == torch.bfloat16)
        except Exception:
            is_bfloat16 = True  # Phòng vệ mặc định cho PaliGemma
