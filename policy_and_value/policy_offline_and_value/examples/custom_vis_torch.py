...
def write_episode_video(
    ...
    # Encode with H.264 for size/speed (Sửa lỗi tương thích Windows NUL thay vì /dev/null)
    new_out_path = out_path.replace(".mp4", "_new.mp4")
    dev_null = "NUL" if os.name == "nt" else "/dev/null"
    ret = os.system(f"ffmpeg -y -i {out_path} -c:v libx264 -crf 18 -preset veryfast {new_out_path} > {dev_null} 2>&1")
    if ret == 0 and os.path.exists(new_out_path):
        logging.info(f"=> Episode {episode_id} generated to: {new_out_path}")
        os.remove(out_path)
    else:
        logging.info(f"=> Episode {episode_id} generated to (OpenCV raw): {out_path}")
...
def main(
    ...
    output_video_dir: str = "C:/TONGHOPTRENLOP/HK6/ML/Project/RISE/policy_and_value/policy_offline_and_value/visualizations",
    ...
    # Ép đường dẫn về dạng chuẩn của hệ điều hành hiện tại (Windows)
    ...
    for observation, actions in tqdm.tqdm(data_iter, total=n_batches, desc="Processing batches"):
        ...
        with torch.no_grad():
            ...
            # [ABLATION STUDY] Nếu bật headview_only, ta "bịt mắt" (zero-out) các camera cổ tay trong quá trình mô hình suy luận!
            if headview_only:
                for cam_name in list(observation_dev.images.keys()):
                    if "wrist" in cam_name or "hand" in cam_name:
                        observation_dev.images[cam_name] = torch.zeros_like(observation_dev.images[cam_name])
